...
public:
    int catMouseGame(vector<vector<int>>& graph) {
        ...
        int t = 0;
        function<int(const tuple<int, int, int>&, int)> bfs = [&](const tuple<int, int, int>& state, int x)
        {
            t = max(t, x);
            if(m.count(state)) return m[state];
            if(get<0>(state) == 0) return 1;
            if(get<0>(state) == get<1>(state)) return -1;            
            ...
            {
                res = -1;
                for(int next : graph[get<0>(state)])
                {
                    auto newState = make_tuple(next, get<1>(state), 0);
                    if(s.count(newState) == 0)
                    {
                        s.insert(newState);
                        res = max(res, bfs(newState, x + 1));
                        s.erase(newState);
                    }
                    else res = max(res, 0);
                }
            ...
            {
                res = 1;
                for(int next : graph[get<1>(state)])
                {
                    auto newState = make_tuple(get<0>(state), next, 1);
                    if(next != 0)
                    {
                        if(s.count(newState) == 0)
                        {
                            s.insert(newState);
                            res = min(res, bfs(newState, x + 1));
                            s.erase(newState);
                        }
                        //else res = min(res, 0);                        
                    }   
        ...
        s.insert(make_tuple(1, 2, 1));
        int res = bfs(make_tuple(1, 2, 1), 1);        
        cout<<"t "<<t<<endl;
        if(res == -1) return 2;
        return res;
    }
