...
public:
    vector<pair<int, int>> pacificAtlantic(vector<vector<int>>& matrix) {
        ...
        m = matrix.size();
        n = matrix[0].size();
        vector<vector<pair<int, int>>> v(m, vector<pair<int, int>>(n, make_pair(-1, -1)));
        for(int i = 0; i < m; ++i)
        {
            v[i][0].first = 1;
            v[i][n - 1].second = 1;
        ...
        for(int i = 0; i < m; ++i)
        {
            for(int j = 0; j < n; ++j)
            {
                if(v[i][j].first != 1)
                {
                    ...
                    visited.emplace(i, j);
                    v[i][j].first = impl(matrix, v, i, j, visited);
                }
                if(v[i][j].second != 1)
                {
                    set<pair<int,int>> visited;
                    visited.emplace(i, j);
                    v[i][j].second = impl(matrix, v, i, j, visited, false);
                }
            }
        }
        ...
        for(int i = 0; i < m; ++i)
        {
            for(int j = 0; j < n; ++j)
            {
                if(v[i][j].first == 1 && v[i][j].second == 1)
                {
    ...
    int impl(vector<vector<int>>& matrix, vector<vector<pair<int, int>>>& v, int i, int j,
             set<pair<int,int>>& visited, bool first = true)
    {
        ...
        for(int k = 0; k < 4; ++k)
        {
            ...
            if(newI >= 0 && newI < m && newJ >= 0 && newJ < n && matrix[newI][newJ] <= matrix[i][j] &&
               visited.find(make_pair(newI, newJ)) == visited.end())
            {
                visited.emplace(newI, newJ);
                if(first)
                {
                    if(v[newI][newJ].first == -1) v[newI][newJ].first = impl(matrix, v, newI, newJ, visited, first);
                    res += v[newI][newJ].first;
                }
                else
                {
                    if(v[newI][newJ].second == -1) v[newI][newJ].second = impl(matrix, v, newI, newJ, visited, first);
                    res += v[newI][newJ].second;
                }
                if(res) break;
            }
        }
...
int main()
{
    ...
    const auto& r = s.pacificAtlantic(a);
    for(const auto& p : r)
    {
